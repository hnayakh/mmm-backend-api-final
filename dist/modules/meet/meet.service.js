...
let MeetService = class MeetService {
    ...
    async createMeet(meeObject) {
        const meetData = meet_entity_1.Meet.createMeet(meeObject.status, meeObject.long, meeObject.lat, meeObject.requestedId, meeObject.requestingId, meeObject.link, meeObject.scheduleTime, meeObject.address, meeObject.type);
        return await this.meet.save(meetData);
    }
    async updateMeet(meetId, meetData) {
        let meetResult = await this.meet.findOne({ id: meetId });
        meetResult.updateMeet(meetData.status, meetData.lat, meetData.long, meetData.requestedId, meetData.requestingId, meetData.link, meetData.scheduleTime, meetData.address, meetData.type);
        ...
        try {
            ...
            if (blockedUser.length > 0) {
                blockedUser.forEach((e) => {
                    ...
                    activeInvites = activeInvites.filter((x) => x.user.id != e.block_who);
                    activeInvites = activeInvites.filter((x) => x.user.id != e.block_whom);
                    activeSent = activeSent.filter((x) => x.user.id != e.block_whom);
                    activeSent = activeSent.filter((x) => x.user.id != e.block_who);
                });
            }
